...
echo rm -r $nird_work/$IM/????_intoNCL
...
# NOT NEEDED: scp Hybrid_To_Pres_new.ncl submit2queue.sh $user@$fram:$fram_work/$IM/
echo ssh $user@$fram sbatch $fram_work/$IM/submit2queue.sh $year $fram_work/$IM/

echo -- 4. FRAM: Copy output from NCL-script to BETZY:
# Input:  FRAM:  /cluster/work/users/tylo/noresm2-wrf/IM_NorESM/1984/
# Output: BETZY: /cluster/work/users/tylo/noresm2-wrf/IM_NorESM/1984/OUTPUT_HIST/NorESM2-MM:1984-01-01_00
echo scp -3 -r $user@$fram:$fram_work/$IM/$year $user@$fram:$betzy_work/$IM/
