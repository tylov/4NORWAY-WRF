...
case $step in
    1) 
        ...
        rem ./remove.sh $nird_work/$IM/????_intoNCL $nird_work/$IM/OUTPUT_HIST/????
    ...
    2)    
        ...
        echo ssh $user@$betzy sbatch $betzy_work/wps/run_metgrid.sh
        ;;
    3)
        rem echo -- 3. BETZY: Run real.exe
        # Input:  namelist.input, met_em*
        # Output:wrfbdy_d01,wrfinput_d01, wrfbdy_d02,wrfinput_d02
        rem echo ssh $user@$betzy sbatch $betzy_work/run_wrf.sh  # Needs SBATCH tuning
        ;;
    4)
        rem echo -- 4. BETZY: Run wrf.exe
